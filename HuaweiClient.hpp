...
class HuaweiAuth{
        public:
                ...
                HuaweiAuth() : user(), password(), confirmedWrong(false) {};
...
class HuaweiClient : public RouterClient{
        ...
        public:
                HuaweiClient(const std::string &server, const std::string &user, const std::string &password) :  credentials(server, user, password), server(server), auth(user, password), connected(false), loggedIn(false) {};
                /**
		 ...
		 */
                ...
                std::string getUserName() {return credentials.user();};
                std::string getPassword() {return credentials.maskedPassword();};
                std::string getServer() {return credentials.server();};
                std::string getPasswordHash() const {return credentials.passwordHash();}
